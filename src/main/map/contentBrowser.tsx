...
export default function ContentBrowser(props: ListContent) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (focusedNode && focusedNode.details) {
                const nodeElement = document.getElementById(focusedNode.details.show_title)
                if (nodeElement) {
                    nodeElement.scrollIntoView({ behavior: "smooth", block: 'center', inline: 'center' })

                    const episodeElement = document.getElementById(focusedNode.details.show_title + focusedNode.details.episode_title)
                    if (episodeElement) {
                        ...
                        <Row style={{ alignItems: 'flex-start' }}>
                            ...
                            <Col style={{ paddingRight: 20, }}>
                                ...
                                {yesRender && <ReactAudioPlayer
                                    ...
                                    onError={() => {
                                        const se: any = { ...selectedEpisodes }
                                        se[podcastName] = { ...se[podcastName], timestamp: selectedEpisode.timestamp, link: audioUrl, loaded: true, error: true }
                                        setSelectedEpisodes(se)
                                    }}
                                    onLoadedMetadata={() => {
                                        const se: any = { ...selectedEpisodes }
                                        se[podcastName] = { ...se[podcastName], timestamp: selectedEpisode.timestamp, link: audioUrl, loaded: true, error: false }
                                        setSelectedEpisodes(se)
                                        startPlayback(title, se[podcastName].timestamp, se[podcastName].link)
                                    }}
                                    controls
                                />
                                }
                                    ...
                                    return <div key={myKey}>
                                        <TimestampEnv>
                                            {thisPodcastTimestamps.map((t: any, ii: number) => {
                                                ...
                                                const selected = () => {
                                                    return selectedEpisodes[podcastName]?.link === t.link && selectedEpisodes[podcastName]?.timestamp === t.timestamp
                                                }
                                                const isError = () => {
                                                    return selectedEpisodes[podcastName]?.link === t.link && selectedEpisodes[podcastName]?.timestamp === t.timestamp && selectedEpisodes[podcastName]?.error
                                                }
                                                ...
                                                if (!t.timestamp) return <div key={ii + 'timestamp'}/>
    ...
    function renderYoutube() {
        ...
        const { description, show_title, episode_title, link, timestamp } = focusedNode.details
