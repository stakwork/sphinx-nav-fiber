...
type Props = {
  handleEdgeTypeClick: (edgeType: string) => void
  schemaLinks: SchemaLink[]
  selectedEdgeTypes: string[]
}

export const EdgeTypes = ({ handleEdgeTypeClick, schemaLinks, selectedEdgeTypes }: Props) => {
  const [showAll, setShowAll] = useState(false)

  const edgeTypes = useMemo(() => {
    const unique = new Set<string>()

    schemaLinks.forEach((l) => {
      if (!l?.edge_type || l.edge_type === 'CHILD_OF') {
        return
      }

      unique.add(l.edge_type)
    })
  ...
  const MAX_ROWS = 4
  const maxVisible = schemasPerRow * MAX_ROWS
  const visible = showAll ? edgeTypes : edgeTypes.slice(0, maxVisible)
  ...
  return (
    <>
      ...
      <PopoverBody>
        ...
        {!showAll && edgeTypes.length > maxVisible && (
