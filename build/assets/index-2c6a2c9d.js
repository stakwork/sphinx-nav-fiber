...
`,ee=c(a)`
  align-items: center;
  cursor: pointer;
`,te=()=>{const{close:s}=g("editNodeName"),u=q({mode:"onChange"}),{watch:r,setValue:n,reset:x,getValues:y}=u,[j,N]=l.useState(!1),[f,v]=l.useState(!1),[o,C]=l.useState(),t=b(),{open:_}=g("removeNode");l.useEffect(()=>(o?n("name",o==null?void 0:o.name):t&&(n("name",t.name),n("image_url",(t==null?void 0:t.image_url)??"")),()=>{x()}),[o,n,x,t]),l.useEffect(()=>{(async()=>{if(!(!t||t.type!=="topic")){v(!0);try{const{data:i}=await $({search:t==null?void 0:t.name}),p=i.find(F=>F.name===t.name);C(p)}catch(i){console.log(i)}finally{v(!1)}}})()},[t]);const I=r("imageInputType"),L=r("name"),d=r("image_url");l.useEffect(()=>{n("imageInputType",k(d))},[d,n]);const D=()=>{s()},m=o||t,R=async()=>{N(!0);const i={["name"]:L.trim(),image_url:d.trim()};try{await H((m==null?void 0:m.ref_id)||"",i);const{updateNode:p}=P.getState();p({...m,...i}),D()}catch(p){console.warn(p)}finally{N(!1)}},A=async()=>{_()},M=y().name&&(o==null?void 0:o.name)!==y().name,V=j||f||!!d&&!I||!d&&!M;return e.jsx(oe,{children:e.jsxs(G,{...u,children:[f?e.jsx(a,{my:24,children:e.jsx(J,{})}):e.jsx(Q,{}),e.jsxs(a,{direction:"row",mb:6,children:[e.jsx(ae,{color:"secondary",disabled:f||!m,onClick:A,size:"large",style:{marginRight:20},variant:"contained",children:"Delete"}),e.jsxs(E,{color:"secondary",disabled:V,onClick:R,size:"large",style:{flex:1},variant:"contained",children:["Save Changes",j&&e.jsx(O,{color:h.BLUE_PRESS_STATE,size:10})]})]})]})})},oe=c(a)`
  padding: 20px;
`,ae=c(E)`
  && {
    color: ${h.primaryRed};
