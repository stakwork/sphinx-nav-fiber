...
`,F=async(t,s,n,o)=>{const b=v(s)?"radar":"add_node",i={};if(s===T)i.media_url=t.source,i.content_type="audio_video";else if(s===D){if(/(?:https?:\/\/)?(?:www\.)?(twitter|x)\.com\/\w+\/status\/(\d+)/s.test(t.source)){const c=/\/status\/(\d+)/,l=t.source.match(c);if(l!=null&&l[1]){const[,S]=l;i.tweet_id=S}}else i.tweet_id=t.source;i.content_type="tweet"}else if(s===U)i.content_type="webpage",i.web_page=t.source;else if(s===O)i.content_type="document",i.text=t.source;else if(s===j){const[,a]=(t.source||"").match(k)||[];if(a)i.source=a,i.source_type=s;else return}else(s===E||s===y||s===R)&&(i.source=t.source,i.source_type=s);t.latitude&&t.longitude&&(i.latitude=t.latitude,i.longitude=t.longitude);let x="",d;ie?d=await ce.enable():d=await ae.enable(),i.pubkey=d==null?void 0:d.pubkey,x=await le();try{const a=await de.post(`/${b}`,JSON.stringify(i),{Authorization:x});if(await ue(n),a.data.project_id&&o(a.data.project_id),a.error){const{message:c}=a.error;throw new Error(c)}}catch(a){if(a.status===402)await pe(n),await F(t,s,n,o);else{let c=m;if(a.status===400)try{const l=await a.json();c=l.message||l.status||(l==null?void 0:l.errorCode)||m}catch{c=m}else a instanceof Error&&(c=a.message||m);throw new Error(c)}}},et=()=>{const[t,s]=f.useState(0),{close:n,visible:o}=ee("addContent"),{setBudget:b}=G(w=>w),{setRunningProjectId:i}=te(w=>w),x=se({mode:"onChange"}),{watch:d,setValue:a,reset:c}=x,[l,S]=f.useState(!1),[H,A]=f.useState("");f.useEffect(()=>()=>{A(""),s(0),c()},[o,c]);const g=d("inputType"),N=d("source"),P=d("source"),W=M(N);f.useEffect(()=>{(async()=>{const h=await ze(P);a("inputType",h)})()},[P,a]);const Y=()=>{n()},I=()=>{s(t===0?v(g)?1:2:t+1)},V=()=>{s(t-1)},q=x.handleSubmit(async w=>{S(!0);try{await F(w,g,b,i),ne("Content Added"),Y()}catch(h){let $=m;if((h==null?void 0:h.status)===400){const _=await h.json();$=_.errorCode||(_==null?void 0:_.status)||m}else h instanceof Error&&($=h.message);A(String($))}finally{S(!1)}});return e.jsx(oe,{id:"addContent",kind:"small",onClose:n,preventOutsideClose:!0,children:e.jsx(re,{...x,children:e.jsxs("form",{id:"add-node-form",onSubmit:q,children:[t===0&&e.jsx(Ze,{allowNextStep:W,onNextStep:I,type:g}),t===1&&v(g)&&e.jsx(Fe,{onNextStep:I,onPrevStep:V,type:g,value:N}),t===2&&e.jsx(Be,{error:H,loading:l,onClick:()=>null,type:g})]})})})};export{et as AddContentModal};
