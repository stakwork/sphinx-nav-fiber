...
`;h.button`
  ...
  cursor: pointer;
  transition: background-color 0.3s ease;
`;const be=v.memo(({type:e,left:t,img:a})=>{var p,c;const r=xe(d=>d.normalizedSchemasByType),n=(p=r[e])==null?void 0:p.primary_color,i=(c=r[e])==null?void 0:c.icon,l=i?`/svg-icons/${i}.svg`:"",o={iconStart:a||l,color:n??x.THING};return s.jsx(At,{style:{left:`${t}%`},children:s.jsx(je,{...o,label:e})})});be.displayName="Marker";const je=v.memo(({iconStart:e,color:t,label:a})=>s.jsx(jt,{color:t,children:e&&s.jsx("img",{alt:a,className:"badge__img",onError:console.log,src:e})}));je.displayName="Badge";const jt=h(b).attrs({direction:"row"})`
  cursor: pointer;
...
`,Et=({markers:e})=>{const t=O(g=>g.tweetPlayingTime),a=O(g=>g.setTweetPlayingTime),r=O(g=>g.setTweetIsPlaying),n=O(g=>g.setTweetDuration),i=O(g=>g.tweetDuration),l=O(g=>g.tweetIsPlaying),[o,p]=v.useState(0),[c,d]=v.useState(0),m=v.useRef(0);v.useEffect(()=>{const g=e.map(A=>A.start?new Date(A.start).getTime():0),w=Math.min(...g),k=Math.max(...g),j=k-w;p(w),d(k),n(j)},[e,n]);const u=v.useRef(null),f=v.useRef(null),y=v.useCallback((g,w)=>{const k=Array.isArray(w)?w[0]:w,j=o+k/100*i;a(j)},[a,o,i]);v.useEffect(()=>{if(l){t>=c&&(a(o),m.current=o),f.current=Date.now();const g=t?(t-o)/i:0,w=Date.now()-g*q;f.current=w;const k=()=>{const j=Date.now()-(f.current??Date.now());if(j<q){const A=j/q,P=o+A*i;a(P),m.current=P,u.current=requestAnimationFrame(k)}else a(c),r(!1),m.current=c,u.current&&cancelAnimationFrame(u.current)};return u.current=requestAnimationFrame(k),()=>{u.current&&cancelAnimationFrame(u.current)}}return m.current=t,()=>{u.current&&(cancelAnimationFrame(u.current),u.current=null)}},[l,o,c,a,i,r,t]);const _=v.useCallback(()=>{t>=c&&(a(o),m.current=o)},[t,c,o,a]);return s.jsxs(It,{children:[s.jsx(ve,{onPlaybackRestart:_}),s.jsx(Pt,{duration:100,handleProgressChange:y,markers:e,playingTime:(t-o)/i*100,tweetPlayingTime:t})]})},It=h(b).attrs({direction:"row",align:"center",justify:"space-between"})`
  padding: 20px;
  background: ${x.BG2};
  height: 96px;
  border-radius: 8px;
...
`,Gt=({nodeId:e,handleAnalyzeClick:t})=>{const[a,r]=v.useState(null);v.useEffect(()=>{(async()=>{try{const u=await ke(e);r(u)}catch(u){console.error(u)}})()},[e]);const n=a?_e(a):null,{text:i,name:l,verified:o,image_url:p,twitter_handle:c}=n||{};let d="";return(n==null?void 0:n.node_type)==="Tweet"?d=`https://x.com/${c}/status/${n==null?void 0:n.tweet_id}`:(n==null?void 0:n.node_type)==="User"&&(d=`https://x.com/${c}`),s.jsx(Vt,{children:s.jsxs(Yt,{children:[s.jsx(Ut,{children:p?s.jsx(Se,{rounded:!0,size:58,src:p,type:"person"}):s.jsx(Ht,{children:s.jsx(it,{})})}),s.jsxs(Kt,{children:[s.jsxs(Xt,{children:[s.jsxs(qt,{children:[l&&s.jsx(Qt,{children:l}),!o&&s.jsx(er,{children:s.jsx(rt,{})})]}),s.jsx(tr,{href:d,target:"_blank",children:s.jsx(at,{})})]}),c&&s.jsxs(Jt,{children:["@",c]}),s.jsx(Zt,{children:i}),a&&s.jsx(Bt,{handleAnalyzeClick:t,node:a})]})]})})},Vt=h(b)`
  background: ${x.ANALYTICS_CARD_BG};
  border-radius: 16px;
  padding: 10px;
  width: 100%;
...
`,rr=v.lazy(()=>De(()=>import("./index-bb31935d.js"),["assets/index-bb31935d.js","assets/index-579ca493.js","assets/index-b4d03b7b.css","assets/index-3ea25930.js","assets/SourcesTableIcon-0c44224f.js","assets/index-a797b5de.js","assets/CheckIcon-4374449d.js","assets/DeleteNodeIcon-cd75ed09.js","assets/SoundIcon-f04d7820.js","assets/SucessFeedBackIcon-36d111e2.js","assets/ClipLoader-9d2b9a7c.js","assets/TableRow-178d7c9e.js"]).then(({TweetAnalyze:e})=>({default:e}))),nr=()=>{const e=O(p=>p.setSelectedTweet),t=O(p=>p.selectedTweet),{tweetId:a}=ge(),[r,n]=v.useState(""),{open:i}=Re("tweetAnalyze"),l=(a==null?void 0:a.split("&"))||[],o=p=>{i(),n(p)};return s.jsx(s.Fragment,{children:s.jsxs(ar,{align:"stretch",basis:"100%",grow:1,shrink:1,children:[l.length>1?s.jsx(ir,{onClick:()=>{o("summary")},children:"Summary Report"}):null,l.map(p=>s.jsx(sr,{className:t===p?"selected":"",onClick:()=>e(p),children:s.jsx(Gt,{handleAnalyzeClick:()=>{o(p)},nodeId:p})},p)),s.jsx(rr,{tweetId:r})]})})},ir=h.button`
  width: 100%;
  background: ${x.ANALYTICS_CARD_BG};
  color: ${x.white};
  padding: 16px 16px;
...
`,ar=h(b)(({theme:e})=>({position:"relative",display:"flex",padding:"0 20px 0 20px",background:"transparent",width:"100%",height:"calc(100vh - 180px)",overflow:"auto",[e.breakpoints.up("sm")]:{width:tt,gap:"10px"}})),sr=h(b)`
  width: 100%;
  margin-bottom: 20px;
  cursor: pointer;
