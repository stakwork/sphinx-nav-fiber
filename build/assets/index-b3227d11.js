...
`,D=async(t,r,s,a)=>{const f=B(s)?"radar":"add_node",o={};if(s===E)o.media_url=t.source,o.content_type="audio_video";else if(s===N){if(/(?:https?:\/\/)?(?:www\.)?twitter\.com\/\w+\/status\/\d+/s.test(t.source)){const c=/\/status\/(\d+)/,x=t.source.match(c);if(x!=null&&x[1]){const[,p]=x;o.tweet_id=p}}else o.tweet_id=t.source;o.content_type="tweet"}else if(s===L)o.content_type="webpage",o.web_page=t.source;else if(s===T)o.content_type="document",o.text=t.source;else if(s===m){const[,i]=(t.source||"").match(A)||[];if(i)o.source=i,o.source_type=s;else return}else(s===b||s===y)&&(o.source=t.source,o.source_type=s);t.latitude&&t.longitude&&(o.latitude=t.latitude,o.longitude=t.longitude);let d="";await Y(async()=>{const i=await K.enable();o.pubkey=i==null?void 0:i.pubkey,d=await Q()});try{const i=await H.post(`/${f}`,JSON.stringify(o),{Authorization:d});if(i.error){const{message:c}=i.error;throw new Error(c)}j(W),r()}catch(i){if(i.status===402&&(await se(a),await J(a),await D(t,r,s,a)),i.status===400){const c=await i.json();j((c==null?void 0:c.status)||C),r()}i instanceof Error&&(j(i.message||C),r())}},Ge=()=>{const[t,r]=h.useState(0),{close:s,visible:a}=re("addContent"),[f]=P(_=>[_.setBudget]),o=te({mode:"onChange"}),{watch:d,setValue:i,reset:c}=o,[x,p]=h.useState(!1);h.useEffect(()=>()=>{r(0),c()},[a,c]);const w=d("inputType"),R=d("source"),I=d("longitude"),O=d("latitude"),k=d("source"),F=U(R);h.useEffect(()=>{i("inputType",we(k))},[k,i]);const G=()=>{s()},v=()=>{r(t+1)},q=()=>{r(t-1)},M=o.handleSubmit(async _=>{p(!0);try{await D(_,G,w,f)}catch{j(C)}finally{p(!1)}});return e.jsx(oe,{id:"addContent",kind:"small",onClose:s,preventOutsideClose:!0,children:e.jsx(ne,{...o,children:e.jsxs("form",{id:"add-node-form",onSubmit:M,children:[t===0&&e.jsx(Be,{allowNextStep:F,onNextStep:v,type:w}),t===1&&e.jsx(e.Fragment,{children:B(w)?e.jsx($e,{onNextStep:v,onPrevStep:q,type:w,value:R}):e.jsx(ze,{form:o,latitude:O,longitude:I,onNextStep:v})}),t===2&&e.jsx(pe,{loading:x,onClick:()=>null,type:w})]})})})};export{Ge as AddContentModal};
