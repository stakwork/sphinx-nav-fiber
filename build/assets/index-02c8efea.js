...
`,Me=({skipToStep:o,allowNextStep:i,onSelectType:c,selectedType:u})=>{const[d]=se(h=>[h.customSchemaFeatureFlag]),[r,x]=f.useState(null),[l,m]=f.useState(!1);f.useEffect(()=>{(async()=>{if(d){m(!0);try{const n=await ne(),g=["about","schema"],p=n.schemas.filter(s=>s.ref_id&&!g.includes(s.type)&&!s.is_deleted).map(s=>({label:we(s.type),value:s.type,action:()=>o("setAttribues")}));x(p)}catch(n){console.warn(n)}finally{m(!1)}}else x([...ue,me])})()},[u,d,o]);const w=h=>{c((h==null?void 0:h.label)||"")};return e.jsxs(t,{children:[e.jsx(t,{align:"center",direction:"row",justify:"space-between",mb:20,children:e.jsx(t,{align:"center",direction:"row",children:e.jsx(Oe,{children:"Select Type"})})}),e.jsx(t,{direction:"row",mb:20,children:e.jsx(xe,{autoFocus:!0,isLoading:l,onSelect:w,options:r})}),e.jsx(t,{children:e.jsx(C,{color:"secondary",disabled:!i,onClick:()=>o("source"),size:"large",type:"button",variant:"contained",children:"Next"})})]})},Oe=y(T)`
  ...
  font-weight: 600;
  font-family: 'Barlow';
`,Pe=async(o,i,c)=>{var h;const u=Object.entries(o).reduce((n,[g,p])=>(p!=null&&p!==""&&(n[g]=p),n),{}),{nodeType:d,typeName:r,sourceLink:x,...l}=u;let m="",w="";d!=="Create custom type"&&await ae(async()=>{const n=await ie.enable();w=n==null?void 0:n.pubkey,m=await ce()});try{const n=await Se(d,l,x,r,m,w);c(o,(h=n==null?void 0:n.data)==null?void 0:h.ref_id)}catch(n){let g=I;if(n.status===400){const p=await n.json();g=p.message||p.errorCode||(p==null?void 0:p.status)||I}else n instanceof Error&&(g=n.message);throw new Error(g)}},Ue=()=>{const[o,i]=f.useState("sourceType"),{close:c,visible:u}=D("addItem"),{open:d}=D("addType"),[r]=G(a=>[a.setBudget]),x=he({mode:"onChange"}),{watch:l,setValue:m,reset:w}=x,[h,n]=f.useState(!1),[g,p]=f.useState(""),[s]=oe(a=>[a.addNewNode]),[j]=re(a=>[a.setSelectedNode]);f.useEffect(()=>()=>{i("sourceType"),w()},[u,w]);const S=l("nodeType"),_=l("typeName"),A=l("sourceLink"),U=l("type");l("title");const M=()=>{c()},E=a=>{p(""),i(a)},J=(a,N)=>{const v=N||`new-id-${Math.random()}`,k=a.nodeType.toLocaleLowerCase(),Q=Object.entries(a).reduce(($,[K,B])=>(B!=null&&B!==""&&($[K]=B),$),{}),P={name:a.typeName??a.name,type:k,label:a.typeName??a.name,node_type:k,id:v,edge_count:0,ref_id:v,x:Math.random(),y:Math.random(),z:Math.random(),date:parseInt((new Date().getTime()/1e3).toFixed(0),10),weight:4,...a.source_link?{source_link:a.source_link}:{},properties:{...Q}};s({nodes:[P],links:[]}),j(P)},W=x.handleSubmit(async a=>{p(""),n(!0);try{await Pe(a,r,J),ge("Item Added"),M()}catch(N){let v=I;if((N==null?void 0:N.status)===400){const k=await N.json();v=k.errorCode||(k==null?void 0:k.status)||I}else N instanceof Error&&(v=N.message);p(String(v))}finally{n(!1)}}),O=a=>{a==="Create custom type"?d():m("nodeType",a)},Y={sourceType:e.jsx(Me,{allowNextStep:!!S,onSelectType:O,selectedType:S,skipToStep:E}),source:e.jsx(Le,{name:_,skipToStep:E,sourceLink:A||"",type:S}),setBudget:e.jsx(Ne,{error:g,loading:h,onClick:()=>null}),createConfirmation:e.jsx(ze,{onclose:M,type:U}),setAttribues:e.jsx(Ie,{handleSelectType:O,nodeType:S,skipToStep:E})},H="small";return e.jsx(fe,{id:"addItem",kind:H,onClose:c,preventOutsideClose:!0,children:e.jsx(ye,{...x,children:e.jsx("form",{id:"add-node-form",onSubmit:W,children:Y[o]})})})};export{Ue as AddItemModal};
