...
`,It=({markers:e})=>{const t=O(g=>g.tweetPlayingTime),a=O(g=>g.setTweetPlayingTime),r=O(g=>g.setTweetIsPlaying),n=O(g=>g.setTweetDuration),i=O(g=>g.tweetDuration),l=O(g=>g.tweetIsPlaying),[o,p]=b.useState(0),[c,d]=b.useState(0),m=b.useRef(0);b.useEffect(()=>{const g=e.map(A=>A.start?new Date(A.start).getTime():0),w=Math.min(...g),k=Math.max(...g),j=k-w;p(w),d(k),n(j)},[e,n]);const u=b.useRef(null),f=b.useRef(null),y=b.useCallback((g,w)=>{const k=Array.isArray(w)?w[0]:w,j=o+k/100*i;a(j)},[a,o,i]);b.useEffect(()=>{if(l){t>=c&&(a(o),m.current=o),f.current=Date.now();const g=t?(t-o)/i:0,w=Date.now()-g*q;f.current=w;const k=()=>{const j=Date.now()-(f.current??Date.now());if(j<q){const A=j/q,P=o+A*i;a(P),m.current=P,u.current=requestAnimationFrame(k)}else a(c),r(!1),m.current=c,u.current&&cancelAnimationFrame(u.current)};return u.current=requestAnimationFrame(k),()=>{u.current&&cancelAnimationFrame(u.current)}}return m.current=t,()=>{u.current&&(cancelAnimationFrame(u.current),u.current=null)}},[l,o,c,a,i,r,t]);const _=b.useCallback(()=>{t>=c&&(a(o),m.current=o)},[t,c,o,a]);return s.jsxs(Et,{children:[s.jsx(be,{onPlaybackRestart:_}),s.jsx(Pt,{duration:100,handleProgressChange:y,markers:e,playingTime:(t-o)/i*100,tweetPlayingTime:t})]})},Et=h(v).attrs({direction:"row",align:"center",justify:"space-between"})`
  padding: 20px;
  background: ${x.BG2};
  height: 96px;
  border-radius: 8px;
...
`,rr=b.lazy(()=>De(()=>import("./index-12224275.js"),["assets/index-12224275.js","assets/index-ad9631ef.js","assets/index-b4d03b7b.css","assets/index-45ec3b30.js","assets/SourcesTableIcon-9a3cfc26.js","assets/index-9193665a.js","assets/CheckIcon-83a45cee.js","assets/DeleteNodeIcon-268ffdef.js","assets/SoundIcon-3370e7c5.js","assets/SucessFeedBackIcon-10b4525f.js","assets/ClipLoader-d09e04e2.js","assets/TableRow-b8d1fe59.js"]).then(({TweetAnalyze:e})=>({default:e}))),nr=()=>{const e=O(p=>p.setSelectedTweet),t=O(p=>p.selectedTweet),{tweetId:a}=ge(),[r,n]=b.useState(""),{open:i}=Re("tweetAnalyze"),l=(a==null?void 0:a.split("&"))||[],o=p=>{i(),n(p)};return s.jsx(s.Fragment,{children:s.jsxs(ar,{align:"stretch",basis:"100%",grow:1,shrink:1,children:[l.length>1?s.jsx(ir,{onClick:()=>{o("summary")},children:"Summary Report"}):null,l.map(p=>s.jsx(sr,{className:t===p?"selected":"",onClick:()=>e(p),children:s.jsx(Gt,{handleAnalyzeClick:()=>{o(p)},nodeId:p})},p)),s.jsx(rr,{tweetId:r})]})})},ir=h.button`
  width: 100%;
  background: ${x.ANALYTICS_CARD_BG};
  color: ${x.white};
  padding: 16px 16px;
